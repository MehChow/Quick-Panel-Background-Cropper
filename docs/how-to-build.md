...
### 3. Populate the user-level Gradle properties

Do not add the upload-key properties to `android/gradle.properties`. That file
is generated by Expo prebuild and `npm run android` intentionally recreates it
with `--clean`.

Add these properties to the user-level Gradle properties file instead:

- macOS/Linux: `~/.gradle/gradle.properties`
- Windows: `%USERPROFILE%\.gradle\gradle.properties`

On macOS/Linux, create the directory and file if needed:
...
The release-signing helper and Gradle both read these user-level properties,
and they survive Expo's clean prebuild.
...
- verify `~/.gradle/gradle.properties` (or `%USERPROFILE%\.gradle\gradle.properties` on Windows)
