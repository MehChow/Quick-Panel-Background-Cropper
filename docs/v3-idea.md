# v3 Idea: Controls + Buttons Support
...
v3 should add support for Good Lock Quick Panel Buttons customization while
preserving the existing Controls-only behavior.

The main v3 use case is applying one selected image seamlessly across both
...
- Do not require v3 to support Buttons-only customization.
...
Future Buttons-only customization should be a separate Advanced target or mode
when it has a focused workflow. It should not be hidden as an edge case inside
Controls + Buttons.
...
Buttons can appear in different shapes and locations:

- Large top pill buttons such as Wi-Fi and Bluetooth
- Smaller round or circular grid buttons
- App-provided buttons
- Device- or region-specific buttons

v3 should treat Buttons as user-defined export boxes inside the shared outer
area. The built-in labels help users name exports, but the screenshot geometry
should drive the real positions.

Open implementation question:

- Whether v3 should start with large pill buttons only, or support both large
  pill buttons and smaller grid buttons in the first Buttons release.

The current product direction leans toward "included Buttons panels" generally,
but this needs implementation planning and UI validation.
...
- Use different defaults for large pill buttons and small round buttons.
...
- Advanced target: Controls only vs Controls + Buttons
...
1. Whether v3 Buttons support includes large pill buttons only or all button
   shapes.
2. How users add/select Buttons panels in the screenshot.
3. How Buttons labels map to export filenames.
4. What default readability treatment Buttons exports should use.
5. Whether the current Advanced calibration screen should branch internally or
