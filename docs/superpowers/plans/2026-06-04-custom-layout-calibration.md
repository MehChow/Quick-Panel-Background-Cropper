...
This plan must not be executed on `main` or `master`. Use the existing feature branch `feat/custom-layout-calibration`, or create it before Task 1 if it does not exist yet.
...
- `E:\Coding_things\Quick-Panel-Background-Cropper\src\features\quick-panel\calibration\components\CustomCalibrationReview.tsx`
  - review step showing the configured panels before save
...
- `E:\Coding_things\Quick-Panel-Background-Cropper\__tests__\export-files.test.ts`

Repo instruction note: do not add new test suites unless explicitly requested by the user. Keep verification lightweight by extending these existing suites only where they directly cover migration, completion rules, preset derivation, and hidden-panel export filtering.
...
export function loadCalibrationProfile(): CalibrationProfile | null {
  ...
  const legacy = loadLegacyCalibrationRect();
  if (!legacy) {
    return null;
  }
...
return {
  ...
  calibrationRect:
    calibrationProfile?.mode === "default-union"
...
- [ ] **Step 6: Extend the existing storage test to cover legacy migration**
...
it("migrates a legacy saved rect into a default-union calibration profile", () => {
  ...
  mmkvStore.__mmkvStore?.set(
    ...
    JSON.stringify({ x: 12, y: 34, width: 200, height: 300, radius: 0 }),
  );
  ...
  expect(loadCalibrationProfile()).toEqual({
    mode: "default-union",
    rect: { x: 12, y: 34, width: 200, height: 300, radius: 0 },
    version: 1,
  });
});
```
...
```bash
git add src/features/quick-panel/model/calibration-profile.ts src/features/quick-panel/model/types.ts src/features/quick-panel/store/storage.ts src/features/quick-panel/store/quick-panel-defaults.ts __tests__/storage.test.ts
...
- [ ] **Step 3: Reject all-hidden custom presets instead of inventing a zero-panel fallback**

```ts
// src/features/quick-panel/model/custom-preset.ts
if (presentIds.length === 0) {
  throw new Error("At least one panel must be present in a custom calibration.");
}
```

- [ ] **Step 4: Make geometry helpers safe for filtered panel orders**
...
- [ ] **Step 5: Route `activePreset` derivation through the calibration profile**
...
- [ ] **Step 6: Extend the existing transitions test to cover custom preset derivation**
...
it("builds a filtered preset from present custom panels only", () => {
  const profile = {
    mode: "custom-panels",
    panels: {
      brightness: { id: "brightness", rect: { x: 10, y: 80, width: 200, height: 50, radius: 20 }, status: "present" },
      buttonBox: { id: "buttonBox", rect: { x: 10, y: 10, width: 200, height: 60, radius: 20 }, status: "present" },
  ...
  const { getPresetFromCalibrationProfile } = require("@/features/quick-panel/store/quick-panel-transitions");

  expect(getPresetFromCalibrationProfile(profile).goodLockOrder).toEqual([
    "buttonBox",
    "brightness",
  ]);
});
...
- [ ] **Step 7: Verify filtered preset derivation**
...
```bash
git add src/features/quick-panel/model/custom-preset.ts src/features/quick-panel/model/types.ts src/features/quick-panel/model/panel-geometry.ts src/features/quick-panel/store/quick-panel-transitions.ts __tests__/quick-panel-transitions.test.ts
...
- Create: `E:\Coding_things\Quick-Panel-Background-Cropper\src\features\quick-panel\calibration\components\CustomCalibrationReview.tsx`
...
export function useCustomCalibrationFlow() {
  ...
  const allPanelsConfigured = Object.values(customCalibrationDraft.panels).every(
    (panel) => panel.status !== "unconfigured",
  );
  const hasPresentPanel = Object.values(customCalibrationDraft.panels).some(
    (panel) => panel.status === "present" && panel.rect,
  );
...
<Button onPress={onNext} disabled={!canContinue}>{isLast ? t("calibration.reviewCustomLayout") : t("common.next")}</Button>
```

- [ ] **Step 4: Add a review screen before save**

```tsx
// src/features/quick-panel/calibration/components/CustomCalibrationReview.tsx
export function CustomCalibrationReview({
  ...
  return (
    <View className="gap-3">
      ...
      <Button onPress={onBack}>{t("common.back")}</Button>
      <Button onPress={onSave}>{t("calibration.saveCustomLayout")}</Button>
    </View>
  );
}
```

- [ ] **Step 5: Branch `CalibrationScreen` by selected calibration mode and review state**

```tsx
// src/features/quick-panel/calibration/CalibrationScreen.tsx
{calibrationMode === "custom-panels" && isReviewStep ? (
  <CustomCalibrationReview
    panels={customCalibrationDraft.panels}
    onBack={leaveReviewStep}
    onSave={saveCalibration}
  />
) : calibrationMode === "custom-panels" ? (
  <CustomCalibrationCanvas
    screenshot={displayedScreenshot}
    rect={currentPanel.rect ?? getSuggestedCalibrationRect(displayedScreenshot)}
...
- [ ] **Step 6: Enforce the spec completion rule during save and review transitions**

```ts
// src/features/quick-panel/store/quick-panel-transitions.ts
export function getCanSaveCustomCalibration(
  panels: Record<PanelId, PanelCalibration>,
) {
  const allPanelsConfigured = Object.values(panels).every(
    (panel) => panel.status !== "unconfigured",
  );
  const hasPresentPanel = Object.values(panels).some(
    (panel) => panel.status === "present" && panel.rect,
  );

  return allPanelsConfigured && hasPresentPanel;
}
```
...
const canEnterReview =
  getCanSaveCustomCalibration(customCalibrationDraft.panels) &&
  displayedScreenshot !== null;

const saveCalibration = () => {
  if (calibrationMode === "custom-panels" && !canEnterReview) {
    setError(translate("errors.customCalibrationIncomplete"));
    return;
  }

  saveCalibrationProfile(customCalibrationDraft);
  router.replace("/");
};
```

- [ ] **Step 7: Add wizard copy for each panel, review step, and invalid all-hidden state**
...
errors: {
  customCalibrationIncomplete: "Configure every panel and keep at least one panel visible before saving.",
  noPanelsToExport: "At least one panel must be visible before export.",
}
```

- [ ] **Step 8: Extend the existing transitions test to enforce the completion rule**
...
it("rejects custom calibration when every panel is hidden", () => {
  ...
  const { getCanSaveCustomCalibration } = require("@/features/quick-panel/store/quick-panel-transitions");

  expect(getCanSaveCustomCalibration(panels)).toBe(false);
});
```

- [ ] **Step 9: Verify the wizard flow manually**

Run: `npm run lint`

Then run: `npx expo run:android`

Expected: the Android app launches, custom layout mode opens a per-panel wizard, the review step appears before save, and save is blocked until every panel is configured and at least one panel is present.

- [ ] **Step 10: Commit**

```bash
git add src/features/quick-panel/calibration/components/CustomCalibrationCanvas.tsx src/features/quick-panel/calibration/components/CustomCalibrationReview.tsx src/features/quick-panel/calibration/components/CustomCalibrationStepper.tsx src/features/quick-panel/calibration/hooks/useCustomCalibrationFlow.ts src/features/quick-panel/calibration/CalibrationScreen.tsx src/features/quick-panel/calibration/hooks/useCalibrationScreen.ts src/features/quick-panel/calibration/components/CalibrationCanvas.tsx i18next/locales/en.ts i18next/locales/zh.ts __tests__/quick-panel-transitions.test.ts
...
- Modify: `E:\Coding_things\Quick-Panel-Background-Cropper\src\features\quick-panel\customize\hooks\useCustomizeScreen.ts`
- Modify: `E:\Coding_things\Quick-Panel-Background-Cropper\src\features\quick-panel\customize\hooks\useCustomizeActions.ts`
...
- Modify: `E:\Coding_things\Quick-Panel-Background-Cropper\__tests__\export-files.test.ts`
...
- [ ] **Step 3: Keep customize hooks aligned with the filtered preset/export flow**

```ts
// src/features/quick-panel/customize/hooks/useCustomizeScreen.ts
const presentPanelIds = activePreset.goodLockOrder;
...
- [ ] **Step 4: Extend the existing export-files test to cover hidden-panel filtering**
...
it("exports only present panels in Good Lock order", async () => {
  const preset = {
    ...
    panels: {
      buttonBox: s25PlusOneUi85Preset.panels.buttonBox,
      brightness: s25PlusOneUi85Preset.panels.brightness,
    },
    visualOrder: ["buttonBox", "brightness"],
  } satisfies QuickPanelPreset;

  await expect(captureAndSaveExports(refs, preset)).resolves.toHaveLength(2);
});
```

- [ ] **Step 5: Update docs and make preview builds explicitly emit an APK**
...
Run: `npm test -- --runInBand __tests__/storage.test.ts __tests__/quick-panel-transitions.test.ts __tests__/export-files.test.ts`

Run: `eas build --platform android --profile preview`

Run: `eas build:run --platform android --latest`

Expected:
- lint passes
- the affected existing test suites pass
- EAS preview build produces an APK artifact suitable for device install
- the latest preview APK installs on an Android device or emulator
- smoke test confirms:
  - default mode still calibrates and exports
  - custom mode reaches the review step
  - a mixed present/hidden custom layout exports only present panels

- [ ] **Step 7: Commit**

```bash
git add src/features/quick-panel/customize/components/QuickPanelPreview.tsx src/features/quick-panel/customize/components/PanelSlice.tsx src/features/quick-panel/customize/components/ExportSurfaces.tsx src/features/quick-panel/customize/hooks/useCustomizeScreen.ts src/features/quick-panel/customize/hooks/useCustomizeActions.ts src/features/quick-panel/customize/services/export-files.ts README.md CALIBRATION_PLAN.md eas.json __tests__/export-files.test.ts
