...
- optional two-screenshot custom calibration with automatic runtime header trim, translucent overlap alignment, and manual seam dragging
...
4. In `Custom layout`, either continue with one screenshot or add a second screenshot, let the app trim its repeated phone header automatically, and align the semi-transparent lower image with the full-width seam control.
...
- automatic runtime-only trimming of the repeated phone header from screenshot 2 before merged calibration
...
- In two-shot custom calibration, screenshot 2 is trimmed automatically at import time and rendered semi-transparent during alignment only before the merged calibration canvas is confirmed.
